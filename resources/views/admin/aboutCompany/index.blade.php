                    ...
                    <table id="companyTable" class="table table-bordered table-striped datatable"  name="companyTable">
                        ...
                        <tbody id="tbody">
                        	...
                        	<tr class="row_{{$company->id}}">
                                <td>{{ $i }}</td>
                                 <td><?php echo str_limit($company->description,120) ?></td>
                                 <td>
                                    <?php echo \App\Link::status($company->id,$company->status)?>
                                </td>
                                <td class="text-nowrap">
                                <?php echo \App\Link::action($company->id)?>
                                </td>
                            </tr>
                        	@endforeach
                        </tbody>
    ...
    <script>
        $(document).ready(function() {
            ...
            $('#companyTable tbody').on( 'click', 'i.fa-trash', function () {
                ...
                aboutCompanyId = $(this).parent().data('id');
                var tableRow = this;
                swal({   
                    title: "Are you sure?",   
                    ...
                    if (isConfirm) {     
                       $.ajax({
                            type: "POST",
                           url : "{{ route('aboutCompany.delete',0) }}",
                            data : {aboutCompanyId:aboutCompanyId},
        ...
        function statusChange(aboutCompanyId) {
            $.ajax({
                    type: "GET",
                    url: "{{ route('aboutCompany.status', 0) }}",
                    data: "aboutCompanyId=" + aboutCompanyId,
                    cache:false,
                    contentType: false,
                    processData: false,
